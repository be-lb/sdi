...
from .mapnik_xml import Datasource,ds_attributes

layers_host= settings.LAYERS_DB['HOST']
layers_dbname= settings.LAYERS_DB['NAME']
layers_user= settings.LAYERS_DB['USER']
layers_password= settings.LAYERS_DB['PASSWORD']

def set_data_source(layer, schema, table):
    ds = Datasource(layer)
    ds_attributes(ds,
        type='postgis', 
        host=layers_host,
        dbname=layers_dbname,
        user=layers_user,
        password=layers_password,
